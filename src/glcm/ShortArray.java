...
public class ShortArray {
	...
	public ShortArray(int... dimensions) throws IllegalArgumentException {
		this.data = null;
		this.numberOfDimensions = dimensions.length;
		
		if(this.numberOfDimensions == 0) {
			throw new IllegalArgumentException("The dimension of a ShortArray must be greater than 0.");
		}
		
		if(!validateSize(dimensions)) {
			throw new IllegalArgumentException("The dimensions of the ShortArray are invalid.");
		}
	...
	public void allocate() {
		this.data = new short[offsets[this.numberOfDimensions]];
	}
	...
	public short get(int... coordinates) /* throws IndexOutOfBoundsException */ {
		/*
	...
	public void set(short value, int... coordinates) /* throws IndexOutOfBoundsException */ {
		/*
		if(!( validDimension(coordinates.length) && validateCoordinates(coordinates) )) {
			throw new IndexOutOfBoundsException("Invalid coordinates.");
		}
	...
	public ShortArray extractRegion(int... coordinates) /* throws IndexOutOfBoundsException */ {
		/*
	...
	*/
	...
	public class ImageIterator implements Iterator {
		private int[] coordinates = new int[ShortArray.this.numberOfDimensions];
		...
		public short get() {
			return ShortArray.this.get(this.coordinates);
		}
		
		public void set(short v) {
			ShortArray.this.set(v, this.coordinates);
		}
		
		public int[] getCoordinates() { return this.coordinates; };
		
		public void move() {
			for(int i = 0; i < ShortArray.this.numberOfDimensions; ++i) {
				if(this.coordinates[i] < ShortArray.this.dimensions[i] - 1) { 
					this.coordinates[i] += 1;
					break;
				} else {
					if(i == ShortArray.this.numberOfDimensions - 1) {
	...
	public class NeighbourhoodIterator implements Iterator {
		...
		private int[] minCoordinates = new int[ShortArray.this.numberOfDimensions];
		private int[] maxCoordinates = new int[ShortArray.this.numberOfDimensions];
		private int[] coordinates;
		private boolean trueIfNotAtTheEnd;
		
		public NeighbourhoodIterator(int... radius) {
			if(radius.length == 1) {
				this.radius = new int[ShortArray.this.numberOfDimensions];
		...
		public void setCenterPixel(int... coordinates) {
			...
			for(int i = 0; i < ShortArray.this.numberOfDimensions; ++i) {
				...
				l = this.centerPixel[i] +  this.radius[i];
				if(l >= ShortArray.this.dimensions[i]) l = ShortArray.this.dimensions[i] - 1; 
	...
	public static void main(String[] args) {
		...
		long start, end;
		short v;
		ShortArray a = new ShortArray(S, S, S);
		a.allocate();
		
		for(int k = 0; k < S; ++k) {
			for(int j = 0; j < S; ++j) {
				for(int i = 0; i < S; ++i) {
					a.set((short)a.getOffset(i, j, k), i, j, k);
				}
			}
		}
